    ...
    KEYS.forEach(v => {
        $("#information").append(`
                      <tr class=\"key-item\">
                        ...
                        <button data-key-id="${v.id}" class=\"btn btn-danger btn-sm delete-key \">delete</button>
                        </td>
                      </tr>
                    `);
    });
...
const addKey = () => {
    ...
    let encryptedPwd = encrypt(password, encryptKey);
    let reqConfig = {
...
/* 删除密码 */
$(document).on("click", ".delete-key", (e)=> {
    let d = $(this).attr("data-key-id")
    console.log(e)
...
$(document).on("click",".view-key",()=>{
    let that = this;
    let keyId = $(that).parent().prevAll(".table-key-id").html()
    console.log(keyId)
    let encryptedPwd = $(this).parent().prevAll(".table-key-password").html()
    console.log(encryptedPwd)
    layer.prompt({title: '输入主密钥，并确认', formType: 1}, function(msg, index){
        try
        {

            let password = decrypt(encryptedPwd,msg)
